...
for ($i = 1; $i -le $Runs; $i++) {
    ...
    # Clean build folder only
    if (Test-Path "build") { 
        Write-Host "Cleaning build folder..." -ForegroundColor Yellow
        Remove-Item -Recurse -Force "build" 
    }
    if (Test-Path ".svelte-kit/output") { 
        Write-Host "Cleaning .svelte-kit/output folder..." -ForegroundColor Yellow
        Remove-Item -Recurse -Force ".svelte-kit/output" 
    }
    ...
    $devStartTime = Measure-Command {
        ...
        while ($elapsed -lt $timeout) {
            ...
            if ($output -match "Local.*http://localhost:5173" -or $output -match "ready in") {
...
Performance analysis of Svelte Habit Tracker application build and startup times across $Runs test runs.
