...
try {
    for ($i = 1; $i -le $Runs; $i++) {
        ...
        # Wait and measure file processing time
...
$optimizations = if ($avgHMR -lt 150) {
    "- Performance is excellent - no immediate optimizations needed`n- Monitor performance as project grows`n- Consider performance budgets for larger codebases"
} else {
    "- Consider reducing bundle size`n- Optimize dependency imports`n- Review component splitting strategy`n- Enable Vite optimization features"
}
