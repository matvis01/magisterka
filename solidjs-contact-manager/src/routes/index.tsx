...
export default function Home() {
      ...
      for (const val of allValues) {
        list = list.filter(
          (contact) =>
            ...
            (contact.notes && contact.notes.toLowerCase().includes(val.toLowerCase())) ||
            (contact.tags && contact.tags.some(tag => tag.toLowerCase().includes(val.toLowerCase())))
        )
      }
    }
    if (sortBy() === 'name') list = [...list].sort((a, b) => a.name.localeCompare(b.name))
    else if (sortBy() === 'surname') list = [...list].sort((a, b) => a.surname.localeCompare(b.surname))
  ...
  function handleExport() {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(allContacts(), null, 2))
    ...
    <main class="w-full h-full flex justify-center text-base-content ">
      <div class="w-full max-w-screen-xl flex flex-col items-start gap-10 md:p-10">
        <div class="w-full flex justify-between items-center">
          ...
          <div class="dropdown dropdown-end">
            ...
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
              <li><button onClick={handleExport}>Export Contacts (JSON)</button></li>
              <li>
                <label class="w-full">
                  Import Contacts (JSON)
                  <input type="file" accept="application/json" class="hidden" onInput={handleImport} />
                </label>
              </li>
            </ul>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5 w-full justify-items-center items-center">
          <For each={paginatedContacts()}>{contact =>
            <Contact
              {...contact}
              edit={() => {
        ...
        <div class="flex justify-center w-full mt-6">
          <div class="join">
            ...
            <For each={
              Array.from(
                { length: Math.min(5, Math.ceil(filteredContacts().length / pageSize)) },
                (_, i) => {
